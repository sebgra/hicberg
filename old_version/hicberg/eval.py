...
def get_interval_index(
    chromosome: str = "",
    value: int = None,
    intervals_dict: dict[str, list[(int, int)]] = None,
    chrom_sizes_dict: dict[str, int] = None,
    ...
    out_dict = {k: ([(None, None)], (None, None)) for k in chrom_sizes_dict.keys()}
...
def select_reads(
    ...
    matrix_file: str = "unrescued_map.cool",
    position: int = 0,
    chromosome: str | list[str] = "",
    bin_size: int = 2000,
    chrom_sizes_dict: str = "chromosome_sizes.npy",
    strides: list[int] = [0],
    trans_chromosome: str = None,
    output_dir: str = None,
    trans_position: list[int] = None,
    nb_bins: int = 1,
    random: bool = False,
    auto: int = None,
) -> dict[str, list[(int, int)]]:
    ...
    if not chrom_sizes_path.exists():
        ...
        raise ValueError(
            f"Chromosome sizes file {chrom_sizes_path} does not exist. Please provide existing chromosome sizes file."
        )
    ...
    if not forward_file_path.exists():
        ...
        raise ValueError(
            f"Forward bam file {forward_file_path} does not exist. Please provide existing bam file."
        )

    if not reverse_file_path.exists():

        raise ValueError(
            f"Reverse bam file {reverse_file_path} does not exist. Please provide existing bam file."
        )
    ...
    if not matrix_file_path.exists():
        ...
        raise ValueError(
            f"Matrix file {matrix_file_path} does not exist. Please provide existing matrix file."
        )
    ...
    if auto is None:
        ...
        if trans_chromosome is not None:
            ...
            list_selected_chromosomes = list(chromosome.split()) + list(
                trans_chromosome
            )
    ...
    if auto is not None:
        ...
        dictionary_of_intervals = draw_intervals(
            chrom_sizes_dict=chrom_sizes_path, nb_intervals=auto, bins=bin_size
        )
...
def select_reads_multithreads_bckp(
    ...
    ## Files where selected reads and duplicates are going to be written
    selected_reads_forward = ps.AlignmentFile(
        output_path / f"{forward_file_path.stem}.in.bam",
        "wb",
        template=forward_file_handler,
    )
    selected_reads_reverse = ps.AlignmentFile(
        output_path / f"{reverse_file_path.stem}.in.bam",
        "wb",
        template=reverse_file_handler,
    )

    ## Files where non selected reads are going to be written
    depleted_reads_forward = ps.AlignmentFile(
        output_path / f"{forward_file_path.stem}.out.bam",
        "wb",
        template=forward_file_handler,
    )
    depleted_reads_reverse = ps.AlignmentFile(
        output_path / f"{reverse_file_path.stem}.out.bam",
        "wb",
        template=reverse_file_handler,
    )
...
def select_reads_multithreads(
    bam_couple: tuple[str, str],
    interval_dictionary: dict[str, (int, int)],
    chrom_sizes_dict: str = "chromosome_sizes.npy",
    output_dir: str = None,
) -> None:
    """
...
def get_intervals_proportions(
    chrom_sizes_dict: str = "chromosome_sizes.npy", nb_intervals: int = 1
) -> dict[str, float]:
    """
    ...
    chr_proportions = {
        k: (v / tot_genome_length)
        for (k, v) in zip(chrom_sizes.keys(), chrom_sizes.values())
    }

    # Draw chromosomes considering their relative proportion in genome
    proportions_choice = choice(
        a=list(chr_proportions.keys()),
        size=nb_intervals,
        replace=True,
        p=list(chr_proportions.values()),
    )
...
def get_chromosomes_intervals(
    ...
    intervals = [
        (i * bins, (i + 1) * bins) for i in range(2, nb_bins - 1)
    ]  # -1 to exclude last bin to avoid anchor position error while constructing maps with cooler
...
def draw_intervals(
    ...
    intervals_proportion = get_intervals_proportions(
        chrom_sizes_dict=chrom_sizes_dict, nb_intervals=nb_intervals
    )
    ...
    for chrom in intervals_proportion.keys():
        ...
        candidate_intervals = get_chromosomes_intervals(
            chrom_sizes_dict=chrom_sizes_dict, chromosome=chrom, bins=bins
        )
        picked_interval_indexes = np.random.choice(
            np.arange(len(candidate_intervals)),
            size=intervals_proportion[chrom],
            replace=False,
        )

        selected_intervals[chrom] = [
            candidate_intervals[idx] for idx in picked_interval_indexes
        ]
...
def get_boundaries(
    ...
    area_to_search = np.append(
        np.arange(
            2 * bins, chrom_sizes.get(chromosome) - 3 * bins, bins
        ),  # np.arange(0, cs_disctionary.item().get(chromosome) - bin_size, bin_size)
...
def check_emptiness(
    intervals: dict[str, list[(int, int)]], matrix: cooler.Cooler = None
) -> bool:
    """
    ...
    for chrom in intervals.keys():
        for interval in intervals[chrom]:
            if (
                matrix.matrix(balance=False)
                .fetch((chrom, interval[0], interval[1]))
                .sum()
                == 0
            ):
                return True
...
def get_TP_table(
    ...
    df_pattern_recall: pd.DataFrame,
    chromosome: str,
    bin_size: int,
    jitter: int = 0,
    ...
    before_after_left = _overlap_intervals_legacy(
        starts1=df_1["start1"],
        ends1=df_1["end1"],
        starts2=df_2["start1"],
        ends2=df_2["end1"],
    ...
    before_after_right = _overlap_intervals_legacy(
        starts1=df_1["start2"],
        ends1=df_1["end2"],
        starts2=df_2["start2"],
        ends2=df_2["end2"],
    ...
    for i in range(selection.shape[0]):
        lines_before.append(
            df_pattern[df_pattern["chrom1"] == chromosome].iloc[selection[i][0]]
        )
        lines_after.append(
            df_pattern_recall[df_pattern_recall["chrom1"] == chromosome].iloc[
                selection[i][1]
            ]
        )
    ...
    final_before.rename(
        columns={
            "chrom1": "chrom1_before",
            "start1": "start1_before",
            "end1": "end1_before",
            "chrom2": "chrom2_before",
            "start2": "start2_before",
            "end2": "end2_before",
            "score": "score_before",
        },
        inplace=True,
    ...
    true_positives_table = pd.concat(
        [final_before, final_after], axis=1, ignore_index=False
    )
...
def get_FN_table(
    ...
    _left, FN_left, FP_left = overlap_intervals_outer(
        starts1=df_1["start1"],
        ends1=df_1["end1"],
        starts2=df_2["start1"],
        ends2=df_2["end1"],
    ...
    _right, FN_right, FP_right = overlap_intervals_outer(
        starts1=df_1["start2"],
        ends1=df_1["end2"],
        starts2=df_2["start2"],
        ends2=df_2["end2"],
...
def get_recall(
    ...
    TP = get_TP_table(
        ...
        df_pattern_recall=df_pattern_recall,
        chromosome=chromosome,
        bin_size=bin_size,
        jitter=jitter,
        threshold=threshold,
...
def get_top_pattern(
    file: str = None, top: int = 10, threshold: float = 0.0, chromosome: str = None
) -> pd.DataFrame:
    """
...
def hicberg_benchmark_cmd_generator(
    file: str = None,
    top: int = 10,
    threshold: float = 0.0,
    chromosome: str = None,
    ...
    df = get_top_pattern(
        file=file, top=top, threshold=threshold, chromosome=chromosome
    ).sort_values(by="start1", ascending=True)

    stride = [
        str(df.iloc[i].start1 - df["start1"].min()) for i in range(0, df.shape[0])
    ]
...
def chromosight_cmd_generator(
    ...
    if matrix_path.suffix != ".cool":
        raise ValueError(
            f"Matrix path {matrix_path} is not a .cool file. Please provide a .cool file."
        )

    if not matrix_path.is_file():
        raise ValueError(
            f"Matrix path {matrix_path} does not exist. Please provide existing matrix path."
        )

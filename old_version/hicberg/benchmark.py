...
def benchmark(
    ...
    if not restriction_map_path.exists():
        raise ValueError(
            f"Restriction map file {restriction_map_path} does not exist. PLease provide an existing restriction map file."
        )
    ...
    if pattern is None or pattern == "-1":
        ...
        if (
            trans_position is not None and trans_position != "-1"
        ):  # -1 usefull when not using trans cases with benchmark calling through Snakemake
        ...
        pre_recall_cmd = hev.chromosight_cmd_generator(
            file=output_data_path / BASE_MATRIX,
            pattern=pattern,
            untrend=trend,
        ...
        df = hev.get_top_pattern(
            file=output_data_path / "original.tsv",
            top=top,
            threshold=threshold,
            chromosome=chromosome,
        ...
        if strides is None or strides == "-1":
            ...
            strides = [
                int(df.iloc[i].start1 - df["start1"].min())
    ...
    for sub_mode in mode.split(","):
        ...
        if not picking_status:
            ...
            # # chunk bam files
            forward_chunks, reverse_chunks = hut.get_chunks(
                output_dir=output_path.as_posix()
            )

            # Multithread part

            coordinates = hev.generate_dict_coordinates(
                matrix_file=output_path / BASE_MATRIX,
                position=position,
                chromosome=chromosome,
                strides=strides,
            ...
            with Pool(processes=cpus) as pool:  # cpus
                ...
                res = pool.map(
                    partial(
                        hev.select_reads_multithreads,
                        interval_dictionary=coordinates,
                        output_dir=output_data_path,
                    ),
            ...
            hio.merge_predictions(
                output_dir=output_data_path, clean=True, stage="benchmark", cpus=cpus
            )

            # TODO : put aside in function
            indexes = hev.get_bin_indexes(
                matrix=base_matrix,
                dictionary=intervals_dictionary,
            )
            picking_status = True
            ...
            hio.build_pairs(
                bam_for=forward_out_path,
                bam_rev=reverse_out_path,
                output_dir=output_data_path,
            )
        ...
        if not learning_status:
            ...
            p1 = Process(
                ...
                kwargs=dict(
                    forward_bam_file=forward_out_path,
                    reverse_bam_file=reverse_out_path,
                    circular=circular,
                    output_dir=output_data_path,
                ),
            )
            p2 = Process(
                target=hst.generate_trans_ps, kwargs=dict(output_dir=output_data_path)
            )
            p3 = Process(
                target=hst.generate_coverages,
                kwargs=dict(
                    forward_bam_file=forward_out_path,
                    reverse_bam_file=reverse_out_path,
                    genome=genome,
        ...
        for _ in range(iterations):
            ...
            for forward_chunk, reverse_chunk in zip(forward_chunks, reverse_chunks):
                if hut.is_empty_alignment(forward_chunk) or hut.is_empty_alignment(
                    reverse_chunk
                ):
            ...
            with Pool(processes=cpus) as pool:  # cpus
                ...
                res = pool.map(
                    partial(
                        ...
                        mode=sub_mode,
                        restriction_map=output_data_path / RESTRICTION_MAP,
                        output_dir=output_data_path,
                    ),
                    zip(forward_chunks, reverse_chunks),
                )
            ...
            hio.build_pairs(
                bam_for="group1.1.out.bam",
                bam_rev="group1.2.out.bam",
                bam_for_rescued="group2.1.rescued.bam",
                bam_rev_rescued="group2.2.rescued.bam",
                mode=True,
                output_dir=output_data_path,
            )
            ...
            pearson = hst.pearson_score(
                original_matrix=base_matrix,
            ...
            if pattern is None or pattern == "-1":
                ...
                rescued_matrix_path = output_data_path / RESCUED_MATRIX
                post_recall_cmd = hev.chromosight_cmd_generator(
                    file=rescued_matrix_path,
                    pattern=pattern,
                    untrend=trend,
                    mode=True,
                    output_dir=output_data_path,
                ...
                true_positives = hev.get_TP_table(
                    df_pattern=output_data_path / "original.tsv",
                    df_pattern_recall=output_data_path / "rescued.tsv",
                    chromosome=chromosome,
                    bin_size=bin_size,
                    jitter=jitter,
                    threshold=threshold,
                )
                false_positives = hev.get_FP_table(
                    df_pattern=output_data_path / "original.tsv",
                    df_pattern_recall=output_data_path / "rescued.tsv",
                    chromosome=chromosome,
                    bin_size=bin_size,
                ...
                false_negatives = hev.get_FN_table(
                    df_pattern=output_data_path / "original.tsv",
                    df_pattern_recall=output_data_path / "rescued.tsv",
                    chromosome=chromosome,
                    bin_size=bin_size,
                ...
                # Get scores
                precision = hev.get_precision(
                    df_pattern=df_original,
                    df_pattern_recall=df_rescued,
                    chromosome=chromosome,
                    bin_size=bin_size,
                    jitter=jitter,
                    threshold=threshold,
                )
                recall = hev.get_recall(
                    df_pattern=df_original,
                    df_pattern_recall=df_rescued,
                    chromosome=chromosome,
                    bin_size=bin_size,
                ...
                # Get plots
                hpl.plot_pattern_reconstruction(
                    table=true_positives,
                    original_cool=output_data_path / BASE_MATRIX,
                    rescued_cool=rescued_matrix_path,
                    chromosome=chromosome,
                    threshold=threshold,
                    case="true_positives",
                    output_dir=output_data_path,
            ...
            hpl.plot_benchmark(
                original_matrix=BASE_MATRIX,
                depleted_matrix=UNRESCUED_MATRIX,
                rescued_matrix=RESCUED_MATRIX,
                chromosomes=chromosome_set,
                output_dir=output_data_path,
            )
            ...
            if not results.exists():
                with open(results, "w") as f_out:
                    ...
                    f_out.write(
                        f"{id_tag}\t{date}\t{chromosome}\t{position}\t{strides}\t{trans_chromosome}\t{trans_position}\t{auto}\t{bins}\t{sub_mode}\t{number_reads}\t{pattern}\t{precision}\t{recall}\t{f1_score}\t{pearson:9.4f}\n"
                    )
                    f_out.close()

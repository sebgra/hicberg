...
def preprocess_pairs(
    pairs_file: str = "all_group.pairs", threshold: int = 1000, output_dir: str = None
) -> None:
    """
    ...
    if not output_dir_path.is_dir():
        raise IOError(
            f"Output directory {output_dir} not found. Please provide a valid path."
        )
    ...
    if not pairs_path.is_file():
        ...
        raise IOError(
            f"Pairs file {pairs_path} not found. Please provide a valid path."
        )
    ...
    with open(processed_pairs_path, "w") as f_out:
        ...
        for line in pairs_handler:
            ...
            if (
                chromosome_for != chromosome_rev
                or np.abs(int(position_rev) - int(position_for)) < threshold
            ):
                continue
                ...
                if int(position_for) < int(position_rev):
                    ...
                    f_out.write(
                        f"{chromosome_for}\t{position_for}\t{position_rev}\t1\n"
                    )
                else:

                    f_out.write(
                        f"{chromosome_for}\t{position_rev}\t{position_for}\t1\n"
                    )
...
def format_chrom_sizes(
    chromosome_sizes: str = "chromosome_sizes.npy", output_dir: str = None
) -> None:
    """
    ...
    if not chrom_size_path.is_file():
        ...
        raise IOError(
            f"Pairs file {chrom_size_path.name} not found. Please provide a valid path."
        )
    ...
    logger.info(
        f"Formated chromosome sizes saved at {chrom_size_bed_path} and {chrom_size_txt_path}"
    )


def get_bed_coverage(
    chromosome_sizes: str = "chromosome_sizes.bed",
    pairs_file: str = "preprocessed_pairs.pairs",
    output_dir: str = None,
) -> None:
    """
    ...
    if not chrom_size_path.is_file():
        ...
        raise IOError(
            f"Pairs file {chrom_size_path} not found. Please provide a valid path."
        )
    ...
    bedtools_cmd = (
        f"bedtools coverage -a {str(chrom_size_path)} -b {str(pairs_path)} -d"
    )
...
def get_bedgraph(bed_coverage: str = "coverage.bed", output_dir: str = None) -> None:
    ...
    if not bed_coverage_path.is_file():
        ...
        raise IOError(
            f"Pairs file {bed_coverage_path.name} not found. Please provide a valid path."
        )
...
def bedgraph_to_bigwig(
    bedgraph_file: str = "coverage.bedgraph",
    chromosome_sizes: str = "chromosome_sizes.txt",
    output_dir: str = None,
) -> None:
    """
    ...
    if not bedgraph_coverage_path.is_file():
        raise IOError(
            f"Pairs file {bedgraph_coverage_path.name} not found. Please provide a valid path."
        )

    chromosome_sizes_path = Path(output_dir, chromosome_sizes)
    if not bedgraph_coverage_path.is_file():
        raise IOError(
            f"Pairs file {chromosome_sizes_path.name} not found. Please provide a valid path."
        )

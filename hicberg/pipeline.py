...
def pipeline(
    ...
    if fq_for == fq_rev:
        ...
        logger.error(
            f"The two provided inputs {fq_for} and {fq_rev} files must be different."
        )
        raise IOError(
            f"The two provided inputs {fq_for} and {fq_rev} files must be different."
        )
    ...
    if start_stage < 1:
        ...
        output_folder = hio.create_folder(
            sample_name=name, output_dir=output_dir, force=force
        )
    ...
    if start_stage < 5:
        ...
        restriction_map = hst.get_restriction_map(
            genome=genome, enzyme=enzyme, output_dir=output_folder
        )
        hst.get_dist_frags(
            genome=genome,
        ...
        p1 = Process(
            ...
            kwargs=dict(
                circular=circular, blacklist=blacklist, output_dir=output_folder
            ),
        )
        p2 = Process(
            target=hst.generate_trans_ps, kwargs=dict(output_dir=output_folder)
        )
        p3 = Process(
            target=hst.generate_coverages,
            kwargs=dict(genome=genome, bins=bins, output_dir=output_folder),
        )
    ...
    if start_stage < 6:
        ...
        if mode == "omics":
            ...
            hom.preprocess_pairs(
                pairs_file="all_group.pairs",
                threshold=distance,
                output_dir=output_folder,
            )
            hom.format_chrom_sizes(
                chromosome_sizes="chromosome_sizes.npy", output_dir=output_folder
            )
            hom.get_bed_coverage(
                chromosome_sizes="chromosome_sizes.bed",
                pairs_file="preprocessed_pairs.pairs",
                output_dir=output_folder,
            )
            hom.get_bedgraph(bed_coverage="coverage.bed", output_dir=output_folder)
            hom.bedgraph_to_bigwig(
                bedgraph_file="coverage.bedgraph",
                chromosome_sizes="chromosome_sizes.txt",
                output_dir=output_folder,
            )
    ...
    if start_stage <= 6:
        ...
        p3 = Process(
            target=hpl.plot_coverages, kwargs=dict(bins=bins, output_dir=output_folder)
        )
        p4 = Process(
            target=hpl.plot_couple_repartition, kwargs=dict(output_dir=output_folder)
        )
        p5 = Process(
            target=hpl.plot_matrix, kwargs=dict(genome=genome, output_dir=output_folder)
        )
